...
export const probeScheduleNotice =
  "服务运行时由内置计时器先从官方来源采集新 URL，再探活 Android+PC 新发现或上次探活可用的官方与历史 URL；失效、未判定、镜像和来源未知链接不再自动探活，证据过期提示只对仍在定时轮换的 URL 显示。普通轮跳过 20 小时内已有有效证据的链接，全量轮忽略 TTL；启用或修改配置会启动新的间隔周期（下一周期生效），运行繁忙时顺延，服务停机后合并为一次补跑。";
