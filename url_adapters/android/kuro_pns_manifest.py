"""Punishing: Gray Raven (PNS) Android JSON adapter."""
...
def collect(
    ...
    """Collect one official Punishing: Gray Raven APK without writing a record."""
    manifest_url = endpoint("kuro", game_id)
    data = json.loads(read_small(manifest_url, timeout))
    resolved_version = version or data.get("version")
    errors = []
    for url in (data.get("primary"), data.get("secondary")):
        ...
        try:
            ...
            return KuroApkCollection(
                ...
                record=record,
                source_name="Kuro official Punishing: Gray Raven Android manifest",
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="战双帕弥什 Android JSON 适配器")
    add_options(parser)
    args = parser.parse_args()
    try:
        path = discover("pns", args.output_root, args.timeout, args.version, args.version_code)
        print(path.resolve())
        return 0
